...
par_gam <- function(i,c,x_t,y_t,knowns,pred_knowns,pred_knowns_se){
  ...
  if (class(b_t) == "try-error") {
    ...
    return(list(predict(b_t,data.frame(x_t=knowns[fold_list==i,c]), se.fit = FALSE,type="response")))
  }
...
for (c in colnames(knowns)) {
  ...
  if (class(b) == "try-error") { b <- try(gam(y~s(x,k=-2),method="ML",select=T,parallel=T))}
  ...
  pred_unknowns <- predict(b,data.frame(x=unknowns[,c]),type="response")

  unknowns[paste(c,"+","RtGAM",sep="")] <- pred_unknowns
